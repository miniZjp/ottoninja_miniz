...
// Check if battery alert is enabled (for boards that support it)
#ifdef CONFIG_BOARD_TYPE_OTTO_NINJA
extern "C" bool is_battery_alert_enabled(void);
#endif
    ...
    if (board.GetBatteryLevel(battery_level, charging, discharging)) {
        ...
        if (low_battery_popup_ != nullptr) {
            if (strcmp(icon, FONT_AWESOME_BATTERY_EMPTY) == 0 && discharging) {
                // Check if battery alert is enabled
                bool show_alert = true;
#ifdef CONFIG_BOARD_TYPE_OTTO_NINJA
                show_alert = is_battery_alert_enabled();
#endif
                if (show_alert) {
                    if (lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) { // Show if low battery popup is hidden and alert enabled
                        ESP_LOGI(TAG, "🔋 Showing low battery popup");
                        lv_obj_remove_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN);
                        app.PlaySound(Lang::Sounds::OGG_LOW_BATTERY);
                    }
                } else {
                    // Alert disabled - hide popup if visible
                    if (!lv_obj_has_flag(low_battery_popup_, LV_OBJ_FLAG_HIDDEN)) {
                        ESP_LOGI(TAG, "🔋 Battery alert disabled - hiding popup");
